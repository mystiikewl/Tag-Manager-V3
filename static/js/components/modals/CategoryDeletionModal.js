...
export class CategoryDeletionModal extends ModalBase {
    ...
    updateWarningDisplay(categoryInfo) {
        ...
        if (hasChildren) {
            ...
            this.addWarningMessage('This category has child categories. Deleting it will also delete all child categories.', 'error');
        } else if (hasProducts) {
    ...
    async handleFormSubmission() {
        ...
        try {
            // Validate form with enhanced details
            const validation = ValidationUtils.validateCategoryDeletionWithDetails(this.categoryInfo);
            if (!validation.canDelete) {
                this.showErrorMessage({ userMessage: validation.errors.join('. ') });
                return;
            }

            // Show confirmation dialog for deletion
            let confirmMessage = `Are you sure you want to delete the category "${this.selectedCategory}"?`;
