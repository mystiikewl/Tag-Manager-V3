    ...
    async handleLevelChange(event) {
        ...
        try {
            ...
            if (level === 1) {
                ...
                this.validationState.parent = true; // Level 1 doesn't need a parent, so it's valid
            } else if (level === 2 || level === 3) {
                await this.showParentField(level);
                this.validationState.parent = false; // Will be validated when parent is selected
            } else {
                this.hideParentField();
